...
class CLog(object):
    ...
    def __init__(self, log_path, token):
        ...
        self._log_file_name = time.strftime("%Y%m%d",time.localtime())
    ...
    def _get_format(self):
        ...
        try:
            ...
            #print "src_file:{0}".format(src_file)
            first = True;
            ...
            while hasattr(f, "f_code"):
                ...
                fmt = '%(asctime)s - {0}:{1} - %(levelname)s - %(name)s - %(message)s'.format(os.path.basename(co.co_filename),f.f_lineno);
                break
                              
        except Exception as ex:
            print "{0}:{1} Exception:{2}".format(__file__, sys._getframe().f_lineno, ex)
            fmt = '%(asctime)s - unknow file:unknow line - %(levelname)s - %(name)s - %(message)s'
            pass
    ...
    def debug(self, *args, **kwargs):
        pass
        log_file = os.path.join(self._log_file_path, self._log_file_name)
        ...
        message = '';
        if len(args) > 0 :
            message = "{0} {1}".format(message, json.dumps(args,ensure_ascii=False)) #如果元组args只有一个可变参数test 则会打印 ('test',) 格式
        if len(kwargs) > 0 :
